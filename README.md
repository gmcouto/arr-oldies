...
- **Audio Language Filtering & Normalization**: Filter media by audio track language supporting ISO 639-1, ISO 639-2, standard language names, and common aliases (e.g. `-l ja`, `-l japanese`, `-l en`).
- **Flexible Filters & Sorting**:
  - Filter by media type (`movie`, `episode`), size range (`--min-size 2GB`, `--max-size 10GB`), age cutoff (`--older-than 1y`, `--newer-than 90d`), or calendar dates (`--before 2023-01-01`).
...
# Scan only Radarr instances with movies older than 2 years
arr-oldies scan --radarr --older-than 2y

# Filter for Japanese audio files larger than 1GB, sorted by size (largest first)
arr-oldies scan -l ja --min-size 1GB -s size --order desc
...
- `-l, --audio-lang <lang>`: Filter by audio language (repeatable, e.g. `-l ja -l en`).
- `--min-size <size>` / `--max-size <size>`: File size bounds (e.g. `500MB`, `2GiB`, `1.5TB`).
- `--older-than <cutoff>` / `--newer-than <cutoff>`: Relative age cutoff (e.g. `30d`, `6m`, `1y`).
...
### 3. `clean`
Safely execute targeted write actions (`--delete`, `--unmonitor`, `--unmonitor-episode`, `--remove`) on media matching filter criteria.
...
# 3. UNMONITOR EPISODES IN SONARR
# Unmonitors specific episodes older than 180 days without deleting the files
arr-oldies clean --unmonitor-episode --sonarr --older-than 180d --execute

# 4. HEADLESS AUTOMATION / CRON EXECUTION
...
- `--unmonitor`: Unmonitor the movie in Radarr or entire series in Sonarr.
- `--unmonitor-episode`: Unmonitor specific matching episode(s) in Sonarr.
